...
class Request {
  constructor(baseRequest) {
    ...
    if (baseRequest) Object.assign(this, baseRequest);
  }
}
...
const getRequestSchema = (request, { name, resultType }) => {
  ...
  if (resultType === "query") {
    if (request && request.length > 0) {
      const properties = (request || []).reduce(
        (prev, { type, name, description, required }) => {
          prev[name] = {
            type: type || "string",
...
function createRequest(yapiRequest) {
  ...
  try {
    ...
    console.log(
      "🚀 -> file: yapi.spec.ts -> line 72 -> createRequest -> error",
      error
    );
  }
  let queryBody = null;
  try {
    ...
    console.log(
      "🚀 -> file: yapi.spec.ts -> line 84 -> createRequest -> error",
      error
    );
  }
  return new Request({
